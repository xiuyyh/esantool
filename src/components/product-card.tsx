...
export function ProductCard({ id, title, country, price: basePrice, salesCount = 0, description, imageUrls, imageHint }: ProductCardProps) {
  ...
  return (
    <Card className={`overflow-hidden glass-card group hover:border-accent transition-all duration-500 rounded-none border-white/5 flex flex-col h-full tech-border relative min-w-0 ${isExclusive ? 'border-accent/40 shadow-[0_0_15px_rgba(0,242,255,0.1)]' : pricing.borderColor}`}>
        ...
        <div className="pt-4 border-t border-accent/10 flex items-center justify-between mt-auto gap-2">
          <div className="flex flex-col shrink-0">
            <div className="flex items-center gap-2">
              <span className={`text-base sm:text-xl font-bold font-mono ${isExclusive ? 'text-accent' : pricing.color}`}>
          ...
          <div className="flex gap-2 min-w-0 shrink">
            ...
            <Button 
              size="sm" 
              className={`h-8 sm:h-9 px-3 sm:px-4 text-[9px] sm:text-[10px] ${isExclusive ? 'bg-accent/20 text-accent border border-accent/40' : pricing.discount > 0 ? 'bg-accent/20 text-accent border border-accent/30 hover:bg-accent/30' : 'bg-accent text-background hover:bg-accent/80'} font-mono font-bold uppercase tracking-widest rounded-none flex-1 sm:flex-none truncate`}
