...
export default function GroupDetailsPage({ params: paramsPromise }: { params: Promise<{ groupId: string }> }) {
  ...
  if (!group || !pricing) return <div className="max-w-5xl mx-auto px-4 py-20 text-center"><h2 className="text-2xl font-bold">Group Not Found</h2><Button className="mt-6" onClick={() => router.push("/")}>Return</Button></div>;
            ...
            <CardContent className="p-8 space-y-6">
              <div className="flex flex-wrap items-center gap-4">
                ...
                <Badge variant="outline" className="border-white/10 uppercase px-3 py-1 font-bold tracking-widest">
                  {group.links?.length || 0} Group Links
                </Badge>
                <Badge variant="outline" className={`uppercase px-3 py-1 font-bold tracking-widest ${pricing.color} ${pricing.borderColor}`}>
                  Quality: {pricing.tier}
                </Badge>
              </div>
              <h2 className="font-headline text-3xl font-bold uppercase tracking-tight">Group Details</h2>
              <div className="p-4 bg-white/[0.02] border border-white/5 space-y-2">
                <div className="flex items-center gap-2 text-[10px] font-bold text-accent uppercase tracking-widest">
                  <Info className="h-3 w-3" /> Price Update Info
                </div>
                <p className="text-[10px] text-muted-foreground leading-relaxed uppercase tracking-tight">
                  This bundle is currently in <span className={pricing.color}>{pricing.label}</span> phase. Prices change automatically based on how many people have bought this bundle (HQ: Best, MQ: Good, LQ: Archive).
                </p>
              </div>
              ...
              <div className="flex flex-col gap-2 border-y border-white/5 py-6">
                ...
                {pricing.discount > 0 && (
                  <div className="bg-accent/10 border border-accent/20 p-2 text-center">
                    <p className="text-[10px] font-bold text-accent uppercase tracking-[0.2em]">-{pricing.discount}% Off Applied</p>
                  </div>
                )}
              </div>
                ...
                {hasAccess ? (
                      ...
                      {(group.links || []).map((link: any, idx: number) => (
                        <div key={idx} className="flex items-center gap-3 p-3 bg-white/[0.03] border border-white/10 rounded-xl group">
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] uppercase text-muted-foreground font-bold mb-1">Link {idx+1}</p>
                  ...
                  <div className="space-y-6">
                    <div className="p-4 bg-white/5 border border-white/10 rounded-xl space-y-3">
                      <div className="flex items-start gap-3">
                        ...
                        <p className="text-[10px] leading-relaxed uppercase tracking-widest text-white/80">
                          These private groups are verified and checked by our automated system.
                        </p>
                      </div>
                      <div className="flex items-start gap-3 border-t border-white/5 pt-3">
                        ...
                        <p className="text-[10px] leading-relaxed uppercase tracking-widest text-muted-foreground">
                          If you find a group that doesn't work, you can file a <span className="text-yellow-500 font-bold">Dispute</span> in your dashboard to get a replacement link.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2 text-[10px] text-muted-foreground uppercase tracking-widest opacity-50">
                      <Lock className="h-3 w-3" /> Secure Delivery
                    </div>
                  </div>
                )}
              </div>
