...
import { Upload, X, Trash2, Edit3, Globe, Lock, Loader2, Plus, Link as LinkIcon, Zap, Layers, RotateCcw, ShieldCheck, Crown } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [groupType, setGroupType] = useState<"bundle" | "exclusive">("bundle");
  ...
  const handleAddGroup = (e: React.FormEvent) => {
    ...
    toast({ title: "Success", description: `${groupType === 'exclusive' ? 'Exclusive' : 'Bundle'} added successfully.` });
    setGroupTitle("");
    setGroupPrice("");
    setGroupDesc("");
    setGroupCountry("");
  ...
  const handleDeleteCountry = (id: string) => {
    if (!db || !confirm("Delete this country?")) return;
    ...
    toast({ title: "Purged", description: "Country removed." });
  };

  const handleDeleteGroup = (id: string) => {
    if (!db || !confirm("Delete this group?")) return;
  ...
  const handleResetSales = (id: string) => {
    if (!db || !confirm("Erase sales count for this item?")) return;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-8 sm:space-y-12 overflow-x-hidden">
      <div className="border-b border-white/5 pb-6">
        <h1 className="font-headline text-2xl sm:text-4xl font-bold uppercase tracking-tight text-white">Marketplace Control</h1>
        <p className="text-muted-foreground mt-1 text-[10px] sm:text-xs uppercase tracking-widest font-mono">Manage Bundles and Exclusive Protocol Nodes</p>
      </div>
        ...
        <div className="lg:col-span-8 space-y-8 min-w-0">
          <Card className="glass-card border-white/5 w-full">
            <CardHeader className="p-4 sm:p-6">
              <CardTitle className="font-headline text-lg sm:text-xl uppercase tracking-widest text-white">Create New Protocol</CardTitle>
            </CardHeader>
            <CardContent className="p-4 sm:p-6 pt-0">
              <form onSubmit={handleAddGroup} className="space-y-6">
                <div className="space-y-4">
                   <Label className="text-[10px] uppercase font-bold text-muted-foreground font-mono">Protocol Type</Label>
                   <Tabs defaultValue="bundle" className="w-full" onValueChange={(val) => setGroupType(val as any)}>
                      <TabsList className="grid w-full grid-cols-2 bg-white/5 h-12">
                        <TabsTrigger value="bundle" className="uppercase text-[10px] font-bold tracking-widest data-[state=active]:bg-primary">Standard Bundle</TabsTrigger>
                        <TabsTrigger value="exclusive" className="uppercase text-[10px] font-bold tracking-widest data-[state=active]:bg-accent data-[state=active]:text-background">Exclusive (1-Off)</TabsTrigger>
                      </TabsList>
                   </Tabs>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-bold text-muted-foreground font-mono">Node Identity</Label>
                    <input type="text" value={groupTitle} onChange={(e) => setGroupTitle(e.target.value)} className="flex h-12 w-full rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" placeholder={groupType === 'exclusive' ? "e.g. Rare Intel Node" : "e.g. VIP Master Bundle"} required />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <Label className="text-[10px] uppercase font-bold text-muted-foreground font-mono">Access Links</Label>
                    <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-accent/5 border border-accent/10">
                      ...
                      <span className="text-[9px] font-bold text-accent uppercase tracking-widest font-mono">
                        {groupLinks.length} Node{groupLinks.length !== 1 ? 's' : ''} Loaded
                      </span>
                    </div>
                  </div>
                    ...
                    {groupLinks.map((link, idx) => (
                      <div key={idx} className="flex flex-col gap-3 p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                          ...
                          <div className="space-y-1">
                            <Label className="text-[9px] uppercase font-bold opacity-40 font-mono">Telegram URL</Label>
                  ...
                  {groupType === 'bundle' && (
                    <Button type="button" variant="outline" size="sm" onClick={addLinkLabel} className="w-full sm:w-auto text-[10px] font-bold uppercase tracking-widest h-10 border-dashed border-white/20">
                      <Plus className="h-3 w-3 mr-2" /> Add Bundle Node
                    </Button>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-bold text-muted-foreground font-mono">Target Region</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground font-mono">Asset Visualization</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground font-mono">Intel Briefing</Label>
                  <Textarea value={groupDesc} onChange={(e) => setGroupDesc(e.target.value)} className="bg-white/5 min-h-[100px] border-white/10" placeholder="Description of nodes included..." required />
                </div>

                <Button type="submit" className={`w-full ${groupType === 'exclusive' ? 'bg-accent text-background hover:bg-accent/90' : 'bg-primary text-white hover:bg-primary/90'} font-bold h-14 uppercase tracking-widest text-xs shadow-lg`}>
                   {groupType === 'exclusive' ? <Crown className="h-4 w-4 mr-2" /> : <Zap className="h-4 w-4 mr-2" />}
                   Deploy {groupType === 'exclusive' ? 'Exclusive Node' : 'Bundle Protocol'}
                </Button>
              </form>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-white/5 w-full">
            ...
            <CardContent className="p-4 sm:p-6 pt-0">
              <form onSubmit={handleAddCountry} className="space-y-4">
                <input type="text" value={countryName} onChange={(e) => setCountryName(e.target.value)} className="flex h-11 w-full rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" placeholder="New Region Name" required />
                <Button type="submit" variant="outline" className="w-full h-11 font-bold uppercase text-[10px] tracking-widest border-white/20">Authorize Region</Button>
              </form>
      ...
      <section className="space-y-6">
        <h2 className="font-headline text-xl sm:text-2xl font-bold uppercase tracking-tight text-white">Active Deployment Registry</h2>
            ...
            <Table>
              <TableHeader className="bg-white/5">
                <TableRow className="border-white/5">
                  ...
                  <TableHead className="font-bold uppercase text-[9px] sm:text-[10px] tracking-widest text-white/60">Type</TableHead>
                  <TableHead className="font-bold uppercase text-[9px] sm:text-[10px] tracking-widest text-white/60">Status/Tier</TableHead>
                  <TableHead className="font-bold uppercase text-[9px] sm:text-[10px] tracking-widest text-white/60 text-center">Sales</TableHead>
                  <TableHead className="font-bold uppercase text-[9px] sm:text-[10px] tracking-widest text-white/60">Price</TableHead>
                  <TableHead className="font-bold text-right uppercase text-[9px] sm:text-[10px] tracking-widest text-white/60">Control</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                    ...
                    <TableRow key={group.id} className="hover:bg-white/5 border-white/5">
                      <TableCell className="font-bold text-sm text-white">
                         <div className="flex flex-col">
                            ...
                            <span className="text-[9px] text-muted-foreground uppercase font-mono">{group.country}</span>
                         </div>
                      </TableCell>
                      <TableCell>
                         {isExclusive ? (
                            <div className="text-[8px] font-bold uppercase px-2 py-0.5 rounded border border-accent/40 bg-accent/10 text-accent inline-flex items-center gap-1">
                               <Crown className="h-2.5 w-2.5" /> EXCLUSIVE
                            </div>
                         ) : (
                            <div className="text-[8px] font-bold uppercase px-2 py-0.5 rounded border border-primary/40 bg-primary/10 text-primary inline-flex items-center gap-1">
                      ...
                      <TableCell>
                        {isExclusive ? (
                           <div className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded border ${group.isSold ? 'border-red-500/30 text-red-500 bg-red-500/5' : 'border-green-500/30 text-green-500 bg-green-500/5'} flex items-center gap-1 w-fit`}>
                              {group.isSold ? 'SOLD' : 'AVAILABLE'}
                           </div>
                        ) : (
                      ...
                      <TableCell className="font-headline font-bold text-sm">
                        <div className="flex flex-col">
                          <span className={isExclusive ? 'text-accent' : pricing.color}>₦{isExclusive ? group.price?.toLocaleString() : pricing.price.toLocaleString()}</span>
                          {!isExclusive && group.price !== pricing.price && (
                             <span className="text-[9px] opacity-40 font-mono line-through">₦{group.price?.toLocaleString()}</span>
                          )}
                        </div>
                      </TableCell>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="glass-card border-white/10 max-w-2xl w-[95vw] sm:w-full max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="font-headline text-lg sm:text-xl uppercase tracking-widest text-white">Update Node Protocol</DialogTitle></DialogHeader>
          {editingGroup && (
            <div className="space-y-6 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <Label className="text-[9px] uppercase font-bold font-mono">Title</Label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <Label className="text-[9px] uppercase font-bold font-mono">Region</Label>
                ...
                <div className="space-y-1">
                  <Label className="text-[9px] uppercase font-bold font-mono">Type</Label>
                  <Select onValueChange={(val) => setEditingGroup({...editingGroup, type: val})} value={editingGroup.type}>
                    <SelectTrigger className="bg-white/5 border-white/10"><SelectValue placeholder="Select Type" /></SelectTrigger>
                    <SelectContent className="glass-card border-white/10">
                       <SelectItem value="bundle" className="uppercase text-[10px] font-bold">Standard Bundle</SelectItem>
                       <SelectItem value="exclusive" className="uppercase text-[10px] font-bold">Exclusive (1-Off)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                   <Label className="text-[9px] uppercase font-bold font-mono">Sales Override</Label>
                   <Input type="number" value={editingGroup.salesCount} onChange={(e) => setEditingGroup({...editingGroup, salesCount: e.target.value})} className="bg-white/5 border-white/10" />
                </div>
                {editingGroup.type === 'exclusive' && (
                  <div className="flex items-center space-x-2 pt-6">
                    <input 
                       type="checkbox" 
                       id="isSold" 
                       checked={editingGroup.isSold} 
                       onChange={(e) => setEditingGroup({...editingGroup, isSold: e.target.checked})}
                       className="w-4 h-4 rounded border-white/10 bg-white/5"
                    />
                    <Label htmlFor="isSold" className="text-[10px] uppercase font-bold tracking-widest">Mark as Sold (Hide from Shop)</Label>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground font-mono">Protocol Links</Label>
                  <span className="text-[9px] font-bold text-accent uppercase font-mono px-2 py-0.5 rounded-full bg-accent/5">
                    {editingGroup.links.length} Active Nodes
                  </span>
                </div>
                <div className="space-y-4">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => setEditingGroup({...editingGroup, links: [...editingGroup.links, {label:"", url:""}]})} className="w-full sm:w-auto text-[10px] uppercase tracking-widest border-white/20">Add Link Node</Button>
              </div>

              <div className="space-y-1">
                <Label className="text-[9px] uppercase font-bold font-mono">Description Brief</Label>
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2">
            ...
            <Button onClick={handleUpdateGroup} className="w-full sm:w-auto bg-primary text-white order-1 sm:order-2">Save Protocol Updates</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
