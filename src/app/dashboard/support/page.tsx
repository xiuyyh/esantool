...
import { ChevronLeft, ExternalLink, Loader2, HelpCircle } from "lucide-react";
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12 lg:py-20 space-y-10">
      ...
      <div className="grid grid-cols-1 gap-10">
          ...
          <CardContent className="pt-4 pb-8 space-y-6 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-white/5">
              <div className="text-left space-y-1 p-4 bg-white/5 rounded-xl border border-white/5">
                ...
                <p className="text-[9px] text-muted-foreground uppercase leading-tight">Send transfer proof for verification.</p>
              </div>
              <div className="text-left space-y-1 p-4 bg-white/5 rounded-xl border border-white/5">
                <p className="text-[8px] font-bold text-accent uppercase tracking-widest">Access</p>
                <p className="text-[9px] text-muted-foreground uppercase leading-tight">Report issues with broken links.</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <section className="space-y-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-white">Common Questions</h2>
          </div>
            ...
            {faqsLoading ? (
              ...
              <Accordion type="single" collapsible className="w-full">
                {faqs.map((faq: any, idx: number) => (
                  <AccordionItem key={faq.id} value={`item-${idx}`} className="border-white/5 px-6">
                    <AccordionTrigger className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-left hover:no-underline hover:text-accent transition-colors py-5">
                      {faq.question}
                    </AccordionTrigger>
                    <AccordionContent className="text-[11px] text-muted-foreground leading-relaxed pb-5 uppercase font-mono">
                      {faq.answer}
                    </AccordionContent>
                  </AccordionItem>
                ))}
              ...
              <div className="p-10 text-center opacity-40">
                <p className="text-[10px] uppercase font-bold tracking-widest">No FAQs found.</p>
              </div>
            )}
          </div>
        </section>
      ...
      <p className="text-center text-[8px] uppercase tracking-widest text-muted-foreground opacity-50 font-mono">
        Include your email address in your message for faster resolution.
      </p>
    </div>
  );
}
